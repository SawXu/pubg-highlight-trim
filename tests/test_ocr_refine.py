...
class OcrRefineTests(unittest.TestCase):
    ...
    def test_detect_events_suppresses_self_elimination_after_any_self_knock(self):
        class FakeCap:
            def release(self):
                pass
    ...
    def test_detect_events_keeps_own_elimination_after_different_own_knock(self):
        class FakeCap:
            def release(self):
                pass
        ...
        self.assertEqual(len(detections), 2)
        self.assertEqual([d.event_key for d in detections], ["own-kill:knock:enemya", "own-kill:eliminate:enemyb"])
